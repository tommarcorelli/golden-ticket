...
- [x] **Notion de furtivité (OPSEC)** : certaines actions "bruyantes" (trop de tickets Kerberoast demandés d'un coup, essais de mot de passe répétés) font monter une jauge d'alerte SOC simulée ; au-delà d'un seuil, un message "l'équipe sécurité a été notifiée" change la fin de mission (pas un échec, mais un score/évaluation différent — introduit la notion réelle de détection, pas juste d'exploitation)
  — livré : jauge visible pendant la mission (6 scénarios), règles de bruit par commande (`js/scenarios.js`, objet `NOISE`), détection à 100% sans faire échouer la mission, stat "OPSEC" au récap de fin, succès 🥷 **Fantôme** pour une mission terminée sans jamais déclencher l'alerte.
...
- [x] **Carte d'attaque façon BloodHound** : graphe SVG (`js/graph.js`, module `AttackGraph`) qui se dessine en direct au fil de l'énumération — comptes/groupes découverts = nœuds, appartenances/ACL/authentifications = arêtes typées (MemberOf, droit abusable, authentification, chemin emprunté), avec badges (SPN, mot de passe cassé/réinitialisé, hash en mémoire) et halo doré sur le compte compromis. Disponible dans les 6 scénarios, avec agrandissement en plein écran (accessible clavier). Le Mode Libre en tire le meilleur parti : les 3 chemins d'attaque valides apparaissent au fur et à mesure, et celui réellement emprunté par le joueur est mis en évidence en doré parmi les autres
