...
- [x] **Mode Blue Team à cas multiples** : le dossier d'incident est désormais tiré au sort parmi deux cas (Kerberoasting ou abus de certificat ADCS/ESC1) à chaque nouvelle investigation, au lieu d'un scénario fixe unique. `SCENARIOS.blueteam.CASES` regroupe les données par cas (journal, indices, verdict, defenses) ; `initState()` en tire un au hasard (ou respecte `forcedCase` si un replay exporté doit reproduire exactement le même cas). Ajouté hors-roadmap le 18 juillet 2026.
