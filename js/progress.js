...
function pruneOrphanIds(obj){
  ...
  // (gère aussi les clés suffixées "id__expert" et "id__budget" des modes challenge)
  const validIds = Object.keys(SCENARIOS);
  Object.keys(obj).forEach(id => {
    const baseId = id.replace(/__expert$/, '').replace(/__budget$/, '');
...
function recordBestTime(scenarioId, elapsed, isExpert, isBudget, cmds){
  const suffix = isBudget ? '__budget' : (isExpert ? '__expert' : '');
  const key = scenarioId + suffix;
  if(!bestTimes[key] || elapsed < bestTimes[key]){
    bestTimes[key] = elapsed;
  }
  if(!runHistory[key]) runHistory[key] = [];
  runHistory[key].push({ time: elapsed, cmds: cmds, date: new Date().toISOString() });
...
function unlockAchievements({ scenarioId, elapsed, hintsUsed, manCount, pathTaken, blueteamCaseId, opsecEnabled, detected, budgetRun }){
  const newlyUnlocked = [];
  const unlock = (id) => {
    if(!achievements[id]){
  ...
  if(budgetRun) unlock('budget_survivor');
