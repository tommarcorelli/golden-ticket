...
SCENARIOS.acl = {
  ...
  counterMeasure:{
    label:'Audit et retrait des ACL dangereuses',
    briefing:"🛡️ Contre-mesure appliquée : l'audit des ACL a retiré le droit ForceChangePassword accordé par erreur au compte Helpdesk sur les comptes à privilèges."
  },
  deepDive:{
  ...
  handle(lower, cmd, m){
    ...
    if(m){
      ...
      if(state.mitigationApplied && hasRight){
        print(`<span class="out-bad">Accès refusé (0x80070005) : droits insuffisants sur ce compte.</span>`);
        print(`<span class="out-dim">💡 Le droit ForceChangePassword accordé par erreur à ${state.user} a été retiré lors de l'audit des ACL.</span>`);
        return true;
      }
      if(!hasRight){
