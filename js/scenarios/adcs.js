...
SCENARIOS.adcs = {
  ...
  counterMeasure:{
    label:'Retrait du drapeau ENROLLEE_SUPPLIES_SUBJECT',
    briefing:"🛡️ Contre-mesure appliquée : le modèle de certificat vulnérable a été audité — le drapeau ENROLLEE_SUPPLIES_SUBJECT a été retiré et l'approbation manager est désormais requise pour l'enrôlement."
  },
  deepDive:{
  ...
  handle(lower, cmd, m){
    ...
    if(m){
      ...
      if(!t.esc1 || state.mitigationApplied){
        if(state.mitigationApplied && t.esc1){
          ...
          print(`<span class="out-dim">💡 Le drapeau ENROLLEE_SUPPLIES_SUBJECT a été retiré du modèle ${tplName} lors de l'audit PKI.</span>`);
          return true;
        }
