...
SCENARIOS.breakglass = {
  ...
  counterMeasure:{
    label:'Mot de passe fort en coffre + rotation',
    briefing:"🛡️ Contre-mesure appliquée : breakglass.admin utilise désormais un mot de passe long et aléatoire, stocké dans un coffre à accès contrôlé (PAM) et tourné après chaque utilisation."
  },
  deepDive:{
  ...
  handle(lower, cmd, m){
    ...
    if(m){
      ...
      if(state.mitigationApplied){
        print(`<span class="out-info">Pulvérisation de mots de passe courants sur breakglass.admin...</span>`);
        print(`<span class="out-bad">Échec : aucun mot de passe testé ne correspond.</span>`);
        print(`<span class="out-dim">💡 breakglass.admin utilise désormais un mot de passe long et aléatoire, stocké dans un coffre (PAM) et tourné après chaque utilisation — plus aucune valeur de dictionnaire ne peut correspondre.</span>`);
        return true;
      }
