...
SCENARIOS.goldenticket = {
  ...
  counterMeasure:{
    label:'Rotation régulière (double) de la clé krbtgt',
    briefing:"🛡️ Contre-mesure appliquée : l'équipe sécurité a mis en place une rotation régulière — et systématiquement double — de la clé krbtgt. La clé que tu répliques via DCSync est déjà obsolète au moment où tu forges ton ticket : elle a été changée deux fois depuis."
  },
  ...
  handle(lower, cmd, m){
    ...
    if(m){
      ...
      if(state.mitigationApplied){
        print(`<span class="out-bad">Échec de l'authentification : ticket rejeté par le contrôleur de domaine.</span>`);
        print(`<span class="out-dim">💡 La clé krbtgt répliquée est déjà périmée — rotation double effectuée depuis. Un Golden Ticket forgé avec l'ancienne clé n'est plus valide nulle part.</span>`);
        return true;
      }
