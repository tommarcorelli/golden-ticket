...
SCENARIOS.gpo = {
  ...
  counterMeasure:{
    label:'Audit et retrait de la délégation GPO copiée par erreur',
    briefing:"🛡️ Contre-mesure appliquée : l'audit des délégations GPO a retiré le droit d'édition copié par erreur sur GPO-AppServers-Baseline. Seule la délégation légitime sur GPO-Imprimantes-Agences (son périmètre d'origine) subsiste pour CORP\\Helpdesk - GPO Imprimantes."
  },
  ...
  handle(lower, cmd, m){
    ...
    if(m){
      ...
      if(state.mitigationApplied && gpoKey === 'GPO-AppServers-Baseline' && hasRight){
        print(`<span class="out-bad">Accès refusé : tu n'as aucun droit d'édition sur cette GPO.</span>`);
        print(`<span class="out-dim">💡 La délégation copiée par erreur sur cette GPO a été retirée lors de l'audit — ton droit s'arrête désormais réellement à GPO-Imprimantes-Agences.</span>`);
        return true;
      }
      if(!hasRight){
        print(`<span class="out-bad">Accès refusé : tu n'as aucun droit d'édition sur cette GPO.</span>`);
