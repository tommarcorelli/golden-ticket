...
SCENARIOS.hybridbridge = {
  id:'hybridbridge',
  cmdBudget:11,  // Mode Budget : nombre de commandes autorisées (objectifs + marge d'exploration/erreur)
  epic:true,
  ...
  counterMeasure:{
    label:'Azure AD Connect traité comme un actif Tier 0',
    briefing:"🛡️ Contre-mesure appliquée : SRV-ADCONNECT01 est désormais traité comme un actif Tier 0, au même titre qu'un contrôleur de domaine. Les droits d'administration locale de svc_web sur ce serveur ont été retirés — restreints aux seuls comptes d'administration Tier 0 dédiés."
  },
  ...
  handle(lower, cmd, m){
    ...
    if(lower === 'dump-adsyncdb'){
      if(state.mitigationApplied){
        print(`<span class="out-bad">Accès refusé (0x80070005) : droits administrateur insuffisants sur ce serveur.</span>`);
        print(`<span class="out-dim">💡 svc_web n'est plus administrateur local de SRV-ADCONNECT01 — ce serveur est désormais traité comme un actif Tier 0, avec des comptes d'administration dédiés.</span>`);
        return true;
      }
