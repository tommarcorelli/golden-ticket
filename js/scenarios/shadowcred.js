...
SCENARIOS.shadowcred = {
  ...
  counterMeasure:{
    label:'Retrait du droit sur msDS-KeyCredentialLink',
    briefing:"🛡️ Contre-mesure appliquée : le droit d'écriture sur msDS-KeyCredentialLink, accordé par erreur, a été retiré lors de l'audit des ACL."
  },
  deepDive:{
  ...
  handle(lower, cmd, m){
    ...
    if(m){
      ...
      if(state.mitigationApplied && hasRight){
        ...
        print(`<span class="out-dim">💡 Ce droit, accordé par erreur, a été retiré lors de l'audit des ACL.</span>`);
        return true;
      }
      if(!hasRight){
        print(`<span class="out-bad">Accès refusé : tu n'as pas de droit d'écriture sur msDS-KeyCredentialLink pour ce compte.</span>`);
