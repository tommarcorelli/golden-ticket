...
SCENARIOS.unconstrained = {
  ...
  counterMeasure:{
    label:'Retrait de la délégation sans contrainte hors DC',
    briefing:"🛡️ Contre-mesure appliquée : WEB01 n'a plus aucune délégation sans contrainte configurée — cette configuration, réservée en principe aux seuls contrôleurs de domaine, a été retirée lors de l'audit."
  },
  deepDive:{
  ...
  handle(lower, cmd, m){
    ...
    if(m){
      ...
      if(state.mitigationApplied){
        ...
        print(`<span class="out-bad">DC01 tente bien de s'authentifier, mais WEB01 n'a plus aucune délégation sans contrainte configurée — son TGT n'est jamais mis en cache ici.</span>`);
        print(`<span class="out-dim">💡 La délégation sans contrainte, réservée aux seuls contrôleurs de domaine, a été retirée de WEB01 lors de l'audit.</span>`);
        return true;
      }
      print(`<span class="out-info">Requête MS-EFSRPC envoyée à DC01 (prétexte : accès à un fichier chiffré sur WEB01)...</span>`);
