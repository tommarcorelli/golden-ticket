...
function bootTerminal(scenarioId, opts){
  ...
  state.mitigationApplied = !!opts.mitigation;
...
function showMissionComplete(elapsed, cmds, hints, newAchievements){
  ...
  const mitigationBtn = document.getElementById('mc-mitigation-btn');
  if(mitigationBtn) mitigationBtn.style.display = (sc.counterMeasure && !state.mitigationApplied) ? 'inline-block' : 'none';
